...
// Get target, score_min, and score_max from categories table for Category_ID 6
$stmt = $pdo->prepare("SELECT target, score_min, score_max FROM categories WHERE Category_ID = 6");
$stmt->execute();
...
$stmt = $pdo->prepare("SELECT MAX(submission_count) as max_submissions FROM (SELECT COUNT(*) as submission_count FROM submissions WHERE Category_ID = 6 GROUP BY User_ID) as counts");
$stmt->execute();
$max_submissions = $stmt->fetchColumn();

// Get the count of submissions for each user for Category_ID 6
$stmt = $pdo->prepare("SELECT User_ID, COUNT(*) as submission_count FROM submissions WHERE Category_ID = 6 GROUP BY User_ID");
$stmt->execute();
...
foreach ($user_scores as $user_id => $score) {
    ...
    if ($exists) {
        ...
        $stmt = $pdo->prepare("UPDATE user_scores SET Cat_F = :score WHERE User_ID = :user_id");
        $stmt->execute([':score' => $score, ':user_id' => $user_id]);
    } else {
        // Insert a new record
        $stmt = $pdo->prepare("INSERT INTO user_scores (User_ID, Cat_F) VALUES (:user_id, :score)");
        $stmt->execute([':user_id' => $user_id, ':score' => $score]);
    }
}

echo "All category F Scores have been updated successfully.";
?>