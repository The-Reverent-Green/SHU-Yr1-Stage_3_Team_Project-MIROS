...
function getEmp(){
    ...
    if(!isset($_POST['filterEmp'])){
        ...
        $stmt = $conn->prepare("SELECT user.*, user_scores.Total_Score FROM user INNER JOIN user_scores ON user.User_ID = user_scores.User_ID");
        $stmt->execute();
        ...
        if($_POST['filterEmp'] != 'Select'){
            ...
            $stmt = $conn->prepare("SELECT user.*, user_scores.Total_Score FROM user INNER JOIN user_scores ON user.User_ID = user_scores.User_ID WHERE user.Role = :role");
