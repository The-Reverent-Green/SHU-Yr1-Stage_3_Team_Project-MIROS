...
$stmt = $pdo->prepare("SELECT target, score_min, score_max FROM categories WHERE Category_ID = 1");
$stmt->execute();
...
foreach ($user_scores as $user_id => $score) {
    ...
    if ($exists) {
        ...
        $stmt = $pdo->prepare("UPDATE user_scores SET Cat_A = :score WHERE User_ID = :user_id");
        $stmt->execute([':score' => $score, ':user_id' => $user_id]);
    } else {
        // Insert a new record
        $stmt = $pdo->prepare("INSERT INTO user_scores (User_ID, Cat_A) VALUES (:user_id, :score)");
        $stmt->execute([':user_id' => $user_id, ':score' => $score]);
    }
}

echo "All category A Scores have been updated successfully.";
?>